defmodule Bumpr.Thread.LinkTest do
  ...
  test "links chain" do
    ...
    link1 = Repo.insert!(%Link{author_id: author1.id, config_id: config.id, message_id: 1, content: "hello"})

    # message id
    assert_raise Ecto.ConstraintError, ~r/links_message_id_index/, fn ->
      Repo.insert!(%Link{author_id: author1.id, config_id: config.id, message_id: 1, content: "hello"})
    end

    # only one nil parent
    assert_raise Exqlite.Error, ~r/Only One null link may exist/, fn ->
      Repo.insert!(%Link{author_id: author1.id, config_id: config.id, message_id: 2, content: "hello"})
    end
    assert Enum.count(Repo.all(Link)) == 1

    link2 = Repo.insert!(%Link{author_id: author1.id, config_id: config.id, message_id: 2, content: "hello", parent_id: link1.id})
    assert Enum.count(Repo.all(Link)) == 2

    assert_raise Ecto.ConstraintError, ~r/links_parent_id_index/, fn ->
      Repo.insert!(%Link{author_id: author1.id, config_id: config.id, message_id: 3, content: "hello", parent_id: link1.id})
    end
    assert Enum.count(Repo.all(Link)) == 2

    assert Repo.reload!(author1).score == 0
    ...
    assert Repo.reload!(author1).score == 1
